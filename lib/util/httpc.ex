defmodule Hipchat.Httpc do
  ...
  def request(method, url, body, headers, options \\ []) do
    ...
    request           = %{method: method, url: url1, headers: headers1, body: body1, options: options1}
    case :hackney.request(method, url1, headers1, body1, options1) do
      {:ok, status, resp_headers, resp_body} -> Response.new(status, convert_resp_headers(resp_headers), resp_body, request)
      {:ok, status, resp_headers}            -> Response.new(status, convert_resp_headers(resp_headers), "", request)
