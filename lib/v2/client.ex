...
defmodule Hipchat.V2.Client do
  ...
  alias Hipchat.{Httpc, Method, Response}
  ...
  @type res_t :: {:ok, Response.t} | {:error, any}
  ...
  @doc false
  @spec send_request(t, Method.t, String.t, list, map) :: res_t
  def send_request(client, method, path, query_params, body) do
    Httpc.request(method, @endpoint <> path, {:form, Map.to_list(body)}, headers(client), httpc_opts(client, query_params))
  end

  defp headers(%__MODULE__{access_token: nil}), do: []
  defp headers(%__MODULE__{access_token: a_t}), do: [{"authorization", "Bearer #{a_t}"}]
